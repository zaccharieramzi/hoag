...
def hoag_lbfgs(
    ...
    for it in range(1, maxiter):
        ...
        if lbfgs:
            ...
            # solve with Broyden method
            sol_broyden = root(
                lambda beta: h_func_grad(beta, lambdak)[1],
                x,
                method='broyden1',
                options=dict(
                    fatol=epsilon_tol * norm_init * np.exp(np.min(old_lambdak) - np.min(lambda0)),
                    ...
                    warm_start_jac=approx_jac,
                ),
            )
            x = sol_broyden.x
            print(np.linalg.norm(h_func_grad(x, lambdak)[1]))
            approx_jac = sol_broyden.hess_inv
        end = time.time()
        if verbose > 0:
